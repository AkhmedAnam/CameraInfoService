...
public class RouterFunctionConfig {
    ...
    public Mono<ServerResponse> getCamerasInfo(ServerRequest serverRequest){
        Flux<CameraInfo> cameraInfoFlux = webClient()
                .get()
                ...
                .flatMap(cameraInfoResponse -> {
                    ...
                    Mono<CameraInfo> map = sourceDataInfoMono.zipWith(tokenDataInfoMono).map(objects -> {
                        TokenDataInfo t2 = objects.getT2();
                        SourceDataInfo t1 = objects.getT1();
                        return CameraInfo.create(cameraInfoResponse.getId(), t1, t2);
                    });
                    return map.subscribeOn(Schedulers.parallel());
                });
        return ok().body(cameraInfoFlux, CameraInfo.class);
    }
